...
List<TvShow> tvShowFromJson(String str) =>
    List<TvShow>.from(json.decode(str).map((x) => TvShow.fromJson(x)));

String tvShowToJson(List<TvShow> data) =>
...
class TvShow {
  ...
  factory TvShow.fromDb(Map<String, dynamic> json) {
    return TvShow(
      id: json["id"] ?? 0,
      url: json["url"] ?? "",
      ...
      genres: (json["genres"] as List<dynamic>? ?? [])
          .map((x) => genreValues.map[x] ?? Genre.DRAMA)
          .toList(),
      status: statusValues.map[json["status"]] ?? Status.RUNNING,
      runtime: json["runtime"],
      ...
      externals: json["externals"] != null
          ...
          : Externals(tvrage: 0, thetvdb: 0, imdb: null),
      ...
      links: json["_links"] != null
          ...
          : Links(
              self: Self(href: ""),
              previousepisode: Episode(href: "", name: ""),
            ),
    );
  }
